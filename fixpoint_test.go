...
func TestQ6(t *testing.T) {
  ...
  for _, f := range []float32{0.25, 1, 10, 0.125} {
    ...
    assert.Equal(t, Q6FromFloat(f*2), q.Mul(two), "multiply by 2")
    square := q.Mul(q)
    assert.Equal(t, Q6FromFloat(f*f), square, "square")
    assert.Equal(t, Q6FromFloat(f), square.Div(q), "div")
  }
}

func TestRotation(t *testing.T) {
  // Create vector to rotate.
  vec1 := mgl32.Vec3{0, 0.832050, 0.554700}
  vec2 := Vec3Q6FromFloat(0, 0.832050, 0.554700)

  // Create rotation quaternion.
  inc1 := mgl32.Quat{0, mgl32.Vec3{0.07, 0, 0}}
  inc1.W = 1.0 - 0.5*(inc1.X()*inc1.X()+inc1.Y()*inc1.Y()+inc1.Z()*inc1.Z())

  inc2 := QuatQ6{Q6FromInt32(0), Vec3Q6FromFloat(0.07, 0, 0)}
  inc2.W = Q6FromInt32(1).Sub(Q6FromFloat(0.5).Mul(inc2.X().Mul(inc2.X()).Add(inc2.Y().Mul(inc2.Y())).Add(inc2.Z().Mul(inc2.Z()))))
